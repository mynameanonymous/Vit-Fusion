...
def prune_and_fuse_layer(neurons, target_size=512):
    # """
    # Prune neurons based on importance using intra-fusion strategy
    # and merge the pruned neurons into the remaining ones using Optimal Transport (OT).
    
    # - neurons: weight matrix of neurons to prune.
    # - target_size: the number of neurons to keep after pruning.
    # """
    # Calculate importance scores (e.g., L1 norm of the weights)
        importance_scores = torch.norm(neurons, dim=1, p=1)
        
        # Sort neurons by importance and prune the least important ones
        sorted_idx = torch.argsort(importance_scores, descending=True)
        pruned_idx = sorted_idx[target_size:]  # indices of neurons to prune
        kept_idx = sorted_idx[:target_size]    # indices of neurons to keep

        # Now, use OT to merge pruned neurons into important ones
        pruned_neurons = neurons[pruned_idx]
        kept_neurons = neurons[kept_idx]
        
        # Perform OT fusion (for simplicity, sum the pruned neurons into the kept neurons)
        for i, pruned in enumerate(pruned_neurons):
            similarity = torch.norm(pruned - kept_neurons, dim=1, p=1)
            min_idx = torch.argmin(similarity)
            kept_neurons[min_idx] += pruned
