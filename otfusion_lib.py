...
def fc_fusion(args: dict, keys: dict, t_in: torch.Tensor , w_0: torch.Tensor, w_1: torch.Tensor, act_0: torch.Tensor, act_1: torch.Tensor, device: torch.device, log: logging.Logger, alpha = 0.5, last_layer = False, is_embed = False, is_vit_fc = False, is_vit_embed = False, fusion_t_w_in=False, wts_eq_t_map = False):
    ...
    if not last_layer:
        ...
        elif args.get('fusion', {}).get('ot_solver', 'emd') == 'fused_gromov_wasserstein':
            log.info(' Using fused_gromov_wasserstein solver to calculate t_map for this layer')
            t_numpy = ot.fused_gromov_wasserstein(mu, nu, M0)
...
def resid_policy(policy, t_resid, t_in, resid_acts, in_acts, log):
    if policy == None:
        ...
        log.info(' "custom" residual connection policy used; propagating the custom weights of t_resid and t_out from MHA')
        y = torch.tensor(2)
        x = torch.tensor(1)
        t_out = torch.add(t_resid, t_in)
        s=1.5
        t_out = torch.div(t_out, s)
